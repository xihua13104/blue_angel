...
void bt_memory_init(bt_memory_type_t type, uint8_t *buf, uint32_t size)
{
    ...
    BT_SHARE_BUFFER_UNLOCK();
    /* add footer */
    bt_memcpy((void *)(buf + size - BT_MM_FOOTER_SIZE), (void *)&bt_mm_footer, BT_MM_FOOTER_SIZE);
...
uint8_t *bt_memory_allocate_packet(bt_memory_type_t type, uint32_t size)
{
    ...
    BT_SHARE_BUFFER_UNLOCK();
    /*update new_mm*/
    bt_memset((uint8_t *)new_mm + sizeof(bt_mm_header_t), 0, new_mm_size - BT_MM_FOOTER_SIZE);
    /*set header info*/
    BT_MM_SET_INFO(new_mm->info, BT_MM_STATE_USING, new_mm_size);
    /*set footer info*/
    bt_memcpy((uint8_t *)new_mm + sizeof(bt_mm_header_t) + new_mm_size - BT_MM_FOOTER_SIZE, &bt_mm_footer, BT_MM_FOOTER_SIZE);
...
void bt_memory_free_packet(bt_memory_type_t type, uint8_t *ptr)
{
    ...
    BT_ASSERT(!bt_memcmp(footer, &bt_mm_footer, BT_MM_FOOTER_SIZE));
