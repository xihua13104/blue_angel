...
static bt_hci_cmd_t bt_gap_init_vendor_cmd_table[] = {
	{BT_HCI_CMD_RESET, 0, NULL},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_pskey_deep_sleep_state), vendor_set_pskey_deep_sleep_state},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_ana_freq_26mhz), vendor_set_ana_freq_26mhz},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_csr_pskey_ana_ftrim), vendor_set_csr_pskey_ana_ftrim},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_csr_pskey_default_transmit_power), vendor_set_csr_pskey_default_transmit_power},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_csr_pskey_max_transmit_power), vendor_set_csr_pskey_max_transmit_power},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_csr_pskey_ble_default_transmit_power), vendor_set_csr_pskey_ble_default_transmit_power},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_csr_pskey_bd_addr), vendor_set_csr_pskey_bd_addr},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_csr_pskey_pcm_config), vendor_set_csr_pskey_pcm_config},	
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_csr_pskey_pcm_formt), vendor_set_csr_pskey_pcm_formt},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_csr_pskey_user_low_jitter_mode), vendor_set_csr_pskey_user_low_jitter_mode},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_hci_nop_disable), vendor_set_hci_nop_disable},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_uart_buad_921600), vendor_set_uart_buad_921600},
	{BT_HCI_CMD_VENDOR_CSR8X11, sizeof(vendor_set_warm_reset), vendor_set_warm_reset},
};
...
static bt_hci_cmd_t bt_gap_init_table[] = {
	...
	{BT_HCI_CMD_WRITE_CLASS_OF_DEVICE, sizeof(class_of_device), class_of_device},
	{BT_HCI_CMD_WRITE_PAGE_TIMEOUT, sizeof(page_timeout), &page_timeout},
	{BT_HCI_CMD_SET_EVENT_MASK, sizeof(event_mask), event_mask},
	{BT_HCI_CMD_WRITE_SIMPLE_PAIRING_MODE, sizeof(ssp_enable), &ssp_enable},
	{BT_HCI_CMD_WRITE_INQUIRY_MODE, sizeof(inquiry_mode), &inquiry_mode},
	{BT_HCI_CMD_WRITE_LE_SUPPORT, sizeof(le_host_support), le_host_support},
...
static bt_hci_cmd_t *bt_gap_init_cmd_pop()
{
	...
	switch (cmd->cmd_code) {
		...
		case BT_HCI_CMD_WRITE_EXT_INQ_RESPONSE:
			...
			cmd->length = sizeof(bt_gap_eir);
			cmd->param = (void *)&bt_gap_eir;
			break;
		default:
			break;
	}
...
bt_status_t bt_gap_init_process(bool is_timeout, uint32_t timer_id, uint32_t data, const void *param)
{
	...
	if (is_timeout) {
		...
		if (need_retry && timer_id == BT_HCI_TIMER_ID_TYPE_A(BT_HCI_CMD_RESET)) {
			if (bt_gap_init_table_index != 1) {
				bt_gap_init_table_index = 14;
			} else {
				bt_gap_init_table_index = 0;
			}
			cmd = bt_gap_init_cmd_pop();
			return bt_hci_cmd_send(*cmd, 1, 100, bt_gap_init_process);
		} else if (timer_id == BT_HCI_TIMER_ID_TYPE_A(BT_HCI_CMD_VENDOR_CSR8X11) && bt_gap_init_table_index != 1) {
			bt_gap_init_table_index = 14;
			cmd = bt_gap_init_cmd_pop();
			return bt_hci_cmd_send(*cmd, 1, 100, bt_gap_init_process);
		}
	...
	if (cmd != NULL) {
		if (cmd->cmd_code == BT_HCI_CMD_RESET) {
			...
			status = bt_hci_cmd_send(*cmd, 0, 2000, bt_gap_init_process);
		}
	} else {
